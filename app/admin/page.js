...
const Admin = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            if (email && password) {
                axios.post("https://funworld-backend-tawny.vercel.app/api/auth/admin", {
                    email: email,
                    password: password
                }).then(res => {
                    if (res.data.admin) {
    ...
    useEffect(() => {
        const fetchSoldTickets = async () => {
            if (isAdminLoggedIn) {
                try {
                    const res = await axios.get("https://funworld-backend-tawny.vercel.app/api/soldtickets");
    ...
    const handleDelete = async(id,index) => {
        ...
        if(!ask)return
        try{
            const res = await axios.delete(`https://funworld-backend-tawny.vercel.app/api/soldtickets?id=${id}`)
            console.log(res)
    ...
    const verifyCredentials = async (email, password) => {
        ...
        try {
            const res = await axios.post("https://funworld-backend-tawny.vercel.app/api/auth/admin", {
                email: email,
                password: password
            });
            if (res.data.admin) {
    ...
    const handleCheckedInChange = async (index, soldTicketId, ticketId) => {
        ...
        for (let i = 0; i < updatedGeneralSoldTicketsArray.length; i++) {
            if (updatedGeneralSoldTicketsArray._id == soldTicketId) {
                const generalTicketToUpdate = updatedGeneralSoldTicketsArray[i].tickets[0]
        ...
        try {
            const res = await axios.put(`https://funworld-backend-tawny.vercel.app/api/soldtickets?id=${soldTicketId}`, { tickets: updatedSoldTicketsArray[index].tickets });
    ...
    const handleButtonClick = (btn) => {
        ...
        if (btn === 'Today') {
            ...
            const filteredArray = generalSoldTicketsArray.filter(item => item.tickets[0].visitDate === formattedCurrentDate);
            setSoldTicketsArray(filteredArray);
        }
        else if (btn === "Tomorrow") {
            const filteredArray = generalSoldTicketsArray.filter(item => item.tickets[0].visitDate === formattedNextDay);
            setSoldTicketsArray(filteredArray);
        }
        else {
                            ...
                            <div className='flex items-center gap-8'>
                                ...
                                <div>
                                    <select
                                        ...
                                        value={selectedButton} onChange={(e) => handleButtonClick(e.target.value)}>
                                ...
                                <tbody>
                                    {soldTicketsArray && soldTicketsArray.map((ticket, index) => (
                                       (ticket._id.toLowerCase().includes(search.toLowerCase()) || ticket.name.toLowerCase().includes(search.toLowerCase()) || ticket.email.toLowerCase().includes(search.toLowerCase()) || ticket.number.toLowerCase().includes(search.toLowerCase()) || ticket.tickets[0].visitDate.toLowerCase().includes(search.toLowerCase()) || findDayOfWeek(ticket.tickets[0].visitDate).toLowerCase().includes(search.toLowerCase())) && <tr key={index}>
                                            <td className="p-1 px-5 border border-gray-300 text-center "><div className='flex flex-row space-x-6 items-center'><div className='cursor-pointer' onClick={async()=>{await handleDelete(ticket._id, index)}}><Image src={'https://funworldblr.s3.amazonaws.com/public/delete.jpg'} width={30} height={30}></Image></div><span className='text-center'>{ticket._id}</span></div></td>
                                            ...
                                            <td className="p-2 border border-gray-300">
                                                {ticket.tickets.map((tik, ind) => (
                                                    <span key={ind} className='flex items-center justify-center'>{tik.visitDate} <br />                                                      {findDayOfWeek(tik.visitDate)}
                                                    </span>
                                                ))}
                                            </td>
                                            <td className="p-2 border border-gray-300 text-center">
                                                <span>Child: {ticket.tickets[0].child}</span> <br />
                                                <span>Adult: {ticket.tickets[0].adult}</span> <br />
                                                <span>Senior: {ticket.tickets[0].senior}</span>
                                            </td>
                                            <td className="p-2 border border-gray-300 text-center"><span className={` font-semibold ${ticket.tickets[0].checkedIn === true ? "text-green-600" : "text-red-600"}`}>
                                                {ticket.payment && (
                                                    <span>
                                                        {ticket.tickets[0].checkedIn === true ? "yes" : "no"}
                                                    </span>
                                                )}
                                            </span>
                                                ...
                                                {ticket.payment &&
                                                    (
                                                        <button
                                                            onClick={() => handleCheckedInChange(index, ticket._id, ticket.tickets[0]._id)}
                                                            className='px-2 py-1 bg-blue-600 text-white font-[600] ml-4 rounded hover:scale-105 transition-all delay-150'>Change</button>
                                                    )
                                                }</td>
