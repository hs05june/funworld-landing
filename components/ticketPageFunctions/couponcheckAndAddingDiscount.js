    ...
    if (
      ...
      if (coupon === "10SUMMEROFF") {
        ...
        if (info.senior > 0 || info.child > 0) {
          setCoupon("");
          window.alert(
            "You can't apply this coupon with Senior Citizens or Children in the ticket, please book a separate ticket for senior citizens / Children or continue without this coupon."
          );
          return;
        }
        ...
        if (info.senior > 0) {
          setCoupon("");
          window.alert(
            "You can't apply this coupon with Senior Citizens in the ticket, please book a separate ticket for senior citizens or continue without this coupon."
          );
          return;
        }
        discount = 0.3;
        ...
        if (info.child > 0 || info.senior > 0 || info.adult < 5) {
          ...
          if (!isFutureDate) {
            window.alert(
              "This offer is only valid on pre-bookings so you cannot use this coupon for today's date. Please choose a future date to apply this offer."
            );
            return;
          } else {
        ...
        if (findDayOfWeek(date) != "Wednesday") {
          setCoupon("");
          window.alert(
            "The offer is only applicable for bookings of Wednesday"
          );
          return;
        } else if (info.child + info.adult + info.senior != 2) {
          setCoupon("");
          window.alert(
            "The offer is only applicable for two Female tickets.(you have more than 2 people in the ticket to apply this offer)."
          );
          return;
        } else {
          if (info.adult > 0) {
